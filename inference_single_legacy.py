...
class InferenceSingle:
    ...
    def _process_segment(self, segment_events: np.ndarray, segment_idx: int) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            # ✅ GPU memory cleanup - prevent accumulation across segments
    ...
    def _merge_segments(self, segment_results: List[Tuple[int, np.ndarray]], 
        ...
        """
        ❌ DEPRECATED: Legacy merge function with memory inefficiency
        
        This function is kept for compatibility but not used in optimized version.
        Stream processing now handles merging inline for better memory efficiency.
    ...
    def process_file(self, input_path: str, output_path: Optional[str] = None, debug: bool = False) -> str:
        ...
        # ✅ Stream processing - avoid segment_results accumulation
        ...
        for segment_idx in range(num_segments):
            ...
            # ✅ Immediate timestamp restoration and memory cleanup
            if len(processed_events) > 0:
                # Restore global timestamps (same formula as original _merge_segments)
                segment_start_global = t_min_original + segment_idx * segment_duration_us
                processed_events[:, 0] += segment_start_global
                all_processed_events.append(processed_events)
            
            # Clean up segment data immediately
            del segment_events, processed_events
        
        # Final merge with same logic as original
        if not all_processed_events:
            self.logger.warning("⚠️  All segments are empty")
            final_events = np.empty((0, 4))
        else:
            # Concatenate all segments (same as original _merge_segments)
            final_events = np.vstack(all_processed_events)
            
            # Sort by timestamp (same as original _merge_segments) 
            sort_indices = np.argsort(final_events[:, 0])
            final_events = final_events[sort_indices]
            
            self.logger.info(f"🔗 Merged {len(all_processed_events)} segments: {len(final_events):,} total events")
